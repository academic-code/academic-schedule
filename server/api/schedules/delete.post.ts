...
export default defineEventHandler(async (event) => {
  ...
  if (schedule.status !== 'DRAFT') {
    throw createError({ statusCode: 400, message: 'Only draft schedules can be deleted' })
  }

  await auth.supabase.from('schedules').delete().eq('id', schedule_id)

  await emitAudit(auth.supabase, auth.userId, 'DELETE', schedule_id, schedule)

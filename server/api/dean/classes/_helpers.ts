...
export async function requireDeanWithSupabase(event: H3Event) {
  ...
  if (!authHeader) throw createError({ statusCode: 401, message: "Missing token" })

  const token = authHeader.replace("Bearer ", "")
  const { data } = await supabase.auth.getUser(token)
  if (!data?.user) throw createError({ statusCode: 401, message: "Unauthorized" })
...
export async function assertTermUnlocked(supabase: any) {
  ...
  if (data?.is_locked) {
    throw createError({ statusCode: 403, message: "Academic term is locked" })
  }
}
